...
class TestFastAPIEndpoints(unittest.TestCase):
    ...
    @patch("llm_server.create_llm")
    def test_generate_endpoint(self, mock_create_llm):
        """Tests the /generate endpoint."""
        mock_create_llm.return_value = VLLMMock()
        print("Patching create_llm with VLLMMock")
...
class TestGenerateEndpoint(unittest.TestCase):
    ...
    @patch("llm_server.create_llm")
    def test_generate_exception_on_llm_call(self, mock_create_llm):
        """Tests generate endpoint for handling exceptions in LLM calls."""
        mock_create_llm.return_value = MagicMock(side_effect=Exception("LLM Exception"))
        print("Patching create_llm with a mock that raises an exception")
